...
validate_required_args() {
  ...
  if [[ -n "${remote_build_cache_type}" && "${remote_build_cache_type}" != 'http' && "${remote_build_cache_type}" != 'gradle-enterprise' && "${remote_build_cache_type}" != 'develocity' ]]; then
    _PRINT_HELP=yes die "ERROR: Invalid value for argument --remote-build-cache-type. Must be one of: 'http', 'gradle-enterprise', or 'develocity'" "${INVALID_INPUT}"
  fi
}
...
validate_build_config() {
  ...
  if [ -z "${tasks}" ]; then
    _PRINT_HELP=yes die "ERROR: Gradle tasks were not found in the build scan. Specify missing argument: --tasks" "${INVALID_INPUT}"
  fi
...
# This overrides explain_collect_gradle_details found in lib/interactive-mode.sh
explain_collect_gradle_details() {
  local text
  IFS='' read -r -d '' text <<EOF
...
collect_remote_build_cache_type() {
  ...
  prompt_for_setting "What is the remote build cache type to use? Must be: 'http', 'gradle-enterprise', 'develocity', or blank for project default." "${remote_build_cache_type}" "${default_remote_cache_type}" remote_build_cache_type

  if [[ -n "${remote_build_cache_type}" && "${remote_build_cache_type}" != 'http' && "${remote_build_cache_type}" != 'gradle-enterprise' && "${remote_build_cache_type}" != 'develocity' ]]; then
    print_bl
    die "ERROR: Invalid value for remote build cache type. Must be one of: 'http', 'gradle-enterprise', 'develocity', or blank for project default" "${INVALID_INPUT}"
  fi
