# Develocity Build Validation Scripts for Gradle
...
If the build scan data is not yet available when the scripts try to fetch it, edit the `network.settings` file and uncomment and update the `max.wait.time` property to set the maximum wait time, using ISO-8601 duration format (e.g., `PT5M` for 5 minutes).
